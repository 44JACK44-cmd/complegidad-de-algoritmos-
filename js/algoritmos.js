...
class ManejadorArchivos {
    ...
    parsearArchivoEntrada(contenido) {
        ...
        for (let i = 0; i < n; i++) {
            if (D[i][i] !== 0) {
                throw new Error(`La diagonal debe contener ceros (D[${i}][${i}] = ${D[i][i]})`);
            }
            ...
            for (let j = 0; j < n; j++) {
                ...
                // Verificar simetría (opcional, según problema)
                if (D[i][j] !== D[j][i]) {
                    console.warn(`Advertencia: Matriz no simétrica en D[${i}][${j}] = ${D[i][j]}, D[${j}][${i}] = ${D[j][i]}`);
                }
            }
        }
    ...
    generarArchivoSalida(datosEntrada, calendario, algoritmo = 'desconocido', tiempoEjecucion = 0) {
        ...
        contenido += `# Costo total: ${this.calcularCostoTotal(datosEntrada, calendario).toFixed(2)}\n`;
        
        return contenido;
    }
    
    calcularCostoTotal(datos, calendario) {
        const { n, D } = datos;
        ...
        for (let i = 0; i < n; i++) {
            ...
            for (let j = 0; j < n; j++) {
                if (i === j) {
                    ...
                    // Generar distancia aleatoria entre 50 y 1000
                    fila.push(Math.floor(Math.random() * 950) + 50);
                }
            }
            D.push(fila);
        }
    ...
    validarCalendarioSalida(calendario, datosEntrada) {
        ...
        for (let fecha = 0; fecha < numFechas; fecha++) {
            ...
            for (let equipo = 0; equipo < n; equipo++) {
                ...
                // Verificar que el valor esté en rango [-n, n] y no sea 0
                if (Math.abs(valor) > n || Math.abs(valor) < 1) {
        ...
        // 5. Verificar repeticiones consecutivas
        for (let equipo = 0; equipo < n; equipo++) {
